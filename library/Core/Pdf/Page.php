...
class Core_Pdf_Page extends Zend_Pdf_Page
{
     ...
     * @param int                    $fontSize
     *
     * @return void|Zend_Pdf_Canvas_Interface
     */
     ...
     * @return mixed
     */
    public function addTable(Core_Pdf $doc, Core_Pdf_Table $table,
                             $posX, $posY, $inContentArea = true)
    {
        //render table
    ...
    /**
     * Get text properties
     * (width, height, [#lines using $max Width]), and warps lines
     * @param $text
     * @param null $maxWidth
     * @return array
     */
     ...
     */
    public function drawRectangle($xStart, $yStart, $xEnd, $yEnd,
    ...
    public function getFontHeight()
    {
        ...
        return ($lineHeight - $lineGap) / $em * $size;
    }
    ...
    private function _getTextWidth($text)
    {
        ...
        $width = array_sum($this->_font->widthsForGlyphs($glyphs))
            / $em * $this->_fontSize;
     ...
     * @param string $text
     * @param int $maxWidth
     * (optional, if not set (auto width) the max width is set by reference)

...
class Core_Form extends Zend_Form
{
    ...
    public function __construct($options = null)
    {
        ...
        if (!$securityOptions) {
            throw new Core_Exception(
                "Security options not set in config"
            );
        }
        ...
        || (isset($registry['captcha']) && $registry['captcha'])) {
            ...
            $element->clearDecorators()
                ...
                ->addDecorator(
                    ...
                    array(
                        'tag' => '<a>',
                        'class' => 'btn-reload captcha-refresh',
    ...
    {
        $securityOptions = Zend_Registry::getInstance()->get(
            'security'
        );
        if (!$securityOptions) {
            throw new Core_Exception(
        ...
        $result = '';
        if ($securityOptions['csrfProtection']) {
            $csrfToken = $this->getElement('csrf_token');
            $result = $csrfToken->render();
        }
        return $result;
    }
